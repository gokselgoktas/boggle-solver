...
namespace boggle {
                ...
                Board const &board, Dictionary const &dictionary)
        {
                ...
                for (unsigned y = 0; y < board.getHeight(); ++y) {
                        for (unsigned x = 0; x < board.getWidth(); ++x) {
                                visitedPoints_.assign(visitedPoints_.size(),
                                    false);
                                flood(board, dictionary, x, y,
                                        std::string());
                        }
                }
        ...
        void DepthFirstSearch::flood(Board const &board,
                Dictionary const &dictionary, int x, int y, std::string string)
        {
                ...
                if (dictionary.hasWordPrefix(string) == false) {
                        ...
                        dictionary.hasWord(string) == true) {
